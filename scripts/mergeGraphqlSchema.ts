...
export function mergeSchema(appName: string){
    ...
    const amplifySchema = parse(readFileSync(join(process.cwd(), `cdk/lib/amplify-export-${appName}/api/${appName}/amplify-appsync-files/schema.graphql`), 'utf8'));
    ...
    const outputFile = join(process.cwd(), `cdk/lib/amplify-export-${appName}/api/${appName}/amplify-appsync-files/schema.graphql`);
